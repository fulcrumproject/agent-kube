...
func TestKamajiClientIntegration(t *testing.T) {
	...
	t.Run("Create Join Token and Get Hash", func(t *testing.T) {
		ctx := context.Background()

		testTenantName := "test-tenant-1746463193"
		...
		// Assertions
		assert.NoError(t, err, "GetTenantCAHash should not return an error")
		assert.NotEmpty(t, caHash, "CA hash should not be empty")
		assert.Contains(t, caHash, "sha256:", "CA hash should be in the format 'sha256:[hash]'")
		...
		// Create a join token
		t.Logf("Creating join token for tenant: %s", testTenantName)
		tokenResponse, err := tenantClient.CreateJoinToken(ctx, testTenantName, 24) // 24 hours validity
